...
export default function Icon({ icon, color, hoverColor, size }: Props) {
  ...
  switch (icon) {
    ...
    case 'boosting_filled':
      svgIcon = (
        <svg
          xmlns="http://www.w3.org/2000/svg"
          aria-label="Server boosting since Apr 16, 2020"
          aria-hidden="false"
          viewBox="0 0 8 12"
          width={`${size}rem`}
          height={`${size}rem`}
  ...
  return <span className={css([styles.container, hoverColor ? styles.hoverContainer : null])}>{svgIcon}</span>;
}
